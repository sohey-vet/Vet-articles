...
3. 【絶対禁止】：「あのコ」「そのコ」「わたし（達）」「お薬」という表現は**絶対に使用しない**でください。主語は自然に省略するか、「ご家族」「動物」「薬」などに適宜言い換えてください。翻訳調の不自然な日本語にならないようにしてください。
4. 【絶対禁止】：「体言止め」の連発、ポエムのようなキザな文章、極端に短い1文ごとの改行は絶対に避けてください。
5. 改行ルール：意味のまとまり（2〜4文程度）ごとに自然な段落を作り、スマートフォンで読みやすい構成にしてください。
6. 出力文字数は「200〜400字程度」に収めてください。
7. 最後に必ず「詳細な実践法はプロフのNoteリンクにて。」等の自然な誘導を一文加えてください。
